...
describe("asyncsse demo", () => {
  ...
  it("with options", async () => {
    ...
    vi.runAllTimers();
    await p;
    expect(JSON.parse(document.getElementById("output").textContent)).toEqual([{ data: "Hello" }, { data: "World" }]);
  });
  ...
  it("empty lines within event", async () => {
    ...
    expect(await run(text)).toEqual([{ data: "Line1" }, { data: "Line2" }, { data: "Line3" }]);
  });
  ...
  it(
    "sync and async onResponse callbacks",
    async () => {
      await run("data: Hello\n\n", "text200", "sync");
      expect(window.syncStatus).toBe(200);
      window.fetch = (t) =>
